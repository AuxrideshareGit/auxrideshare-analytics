...
const isItemActive = (item, pathname) => {
  if (item.href && item.href !== '#' && (pathname === item.href || pathname.startsWith(item.href + '/'))) return true;
  if (item.children) {
...
const MenuItem = ({
  ...
  const pathname = usePathname();
  const Icon = item.icon;
  const isActive = item.href && item.href !== '#' && (pathname === item.href || pathname.startsWith(item.href + '/'));
