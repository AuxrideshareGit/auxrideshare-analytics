  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const result = await apiFetch(`/api/v1/admin/system/otps/?${queryParams}`, { token });
    ...
    <div className="card">
      <div className="p-4 flex flex-wrap items-center gap-4">
        ...
        <div className="relative w-64">
          <input type="text" className="form-input form-input-sm ps-9 w-full" placeholder="Search user or OTP code..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
            ...
            <table className="min-w-full divide-y divide-default-200">
              <thead className="bg-default-150 sticky top-0 z-20 shadow-sm backdrop-blur-md">
                <tr className="text-sm font-normal text-default-700 whitespace-nowrap">
                  ...
                  <th className="px-3.5 py-3 text-start cursor-pointer select-none" onClick={() => handleSort('otp_code')}>OTP Code {currentSortIcon('otp_code')}</th>
                  <th className="px-3.5 py-3 text-start cursor-pointer select-none" onClick={() => handleSort('expires_at')}>Expires At {currentSortIcon('expires_at')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-default-200">
                {loading ? (
                  <tr><td colSpan="4" className="py-10 text-center"><LuLoader className="animate-spin size-6 text-primary mx-auto" /><p className="mt-2 text-default-500">Loading OTPs...</p></td></tr>
                ) : data.length === 0 ? (
                  <tr><td colSpan="4" className="py-10 text-center text-default-500">No OTPs found.</td></tr>
                ) : (
                  data.map((row) => (
                    <tr key={row.id} className="text-default-800 font-normal text-sm hover:bg-default-50 transition-colors">
                      ...
                      <td className="px-3.5 py-3 text-default-600 font-mono tracking-wider relative group">
                        <div className="flex items-center gap-2">
                          <span>{row.otp_code}</span>
                          <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(row.otp_code); }} className="text-default-400 hover:text-primary transition-colors flex-shrink-0" title="Copy OTP Code">
                            <LuCopy className="size-4" />
                          </button>
                        </div>
                      </td>
                      <td className="px-3.5 py-3 whitespace-nowrap text-default-600 text-xs">{formatDate(row.expires_at)}</td>
                    </tr>
                  ))
                )}
              </tbody>
