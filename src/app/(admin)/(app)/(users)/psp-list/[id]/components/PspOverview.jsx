...
const PspOverview = ({ pspId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const result = await apiFetch(`/api/v1/admin/psp-users/${pspId}/commission-history/?${queryParams}`, { token });
        setData(result);
      } catch (error) {
        console.error("Error fetching PSP history:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    fetchData();
  }, [pspId, commPage, payoutPage, limit, token]);
  ...
  const pspUser = data.psp_info || data.psp_user_info || data.affiliate_info || {};
    ...
    <div className="space-y-6">
      ...
      <div className="border border-default-200 rounded-lg bg-white overflow-hidden shadow-sm">
        ...
        <div className="p-6 flex flex-col gap-4">
          <div className="flex">
            ...
            <div className="text-sm text-default-600">{pspUser.name || 'N/A'}</div>
          </div>
          <div className="flex">
            <div className="w-32 font-semibold text-sm text-default-700">Phone:</div>
            <div className="text-sm text-default-600">{pspUser.phone_number || 'N/A'}</div>
          </div>
        </div>
      </div>
      ...
      <div className="card shadow-sm border border-default-200 overflow-hidden">
        <div className="bg-black px-6 py-4 flex items-center">
          ...
          <h3 className="text-white font-medium">Recent Commission Records ({commissions.total_count} total rides)</h3>
        </div>
          ...
          <table className="min-w-full divide-y divide-default-200">
            <thead className="bg-default-50">
              <tr className="text-sm font-semibold text-default-800 whitespace-nowrap">
                ...
                <th className="px-6 py-4 text-start">Reports Combined</th>
            ...
            <tbody className="divide-y divide-default-200 bg-white">
              {commissions.data.length === 0 ? (
                <tr>
                  <td colSpan="9" className="px-6 py-8 text-center text-default-500">
                    No recent commissions found.
                  </td>
                </tr>
              ) : (
                commissions.data.map((comm) => {
                  const isPaid = comm.all_paid || comm.status === 'PAID';
                  return (
                    <tr key={comm.ride_id || comm.ride_request_id || comm.id} className="text-default-700 text-sm hover:bg-default-50">
                      <td className="px-6 py-4 whitespace-nowrap font-medium text-primary">
                        <Link href={`/rider-requests/${comm.ride_request_id || comm.ride_id || ''}`} className="hover:underline">
                          #{comm.rider_request_id || comm.ride_request_id || comm.ride_id || comm.id || '-'}
                        </Link>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap font-bold text-success-600">
                        ${(comm.total_psp_commission || comm.amount || 0).toFixed(2)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-[11px] uppercase font-bold rounded ${
                          isPaid ? 'bg-success text-white' : 'bg-warning text-white'
                        }`}>
                          {isPaid ? 'PAID' : 'PENDING'}
                        </span>
                      </td>
                      <td className="px-6 py-4 truncate max-w-xs" title={comm.pickup_address || comm.ride_details?.pickup_address}>
                        {comm.pickup_address || comm.ride_details?.pickup_address || '-'}
                      </td>
                      <td className="px-6 py-4 truncate max-w-xs" title={comm.dropoff_address || comm.ride_details?.dropoff_address}>
                        {comm.dropoff_address || comm.ride_details?.dropoff_address || '-'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {comm.driver_name || comm.ride_details?.driver_name || '-'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {pspUser.name || '-'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {comm.reports_count || 1} report(s)
                      </td>
          ...
          <table className="min-w-full divide-y divide-default-200">
            <thead className="bg-default-50">
              <tr className="text-sm font-semibold text-default-800 whitespace-nowrap">
                ...
                <th className="px-6 py-4 text-start">Processed Date</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-default-200 bg-white">
              {payouts.data.length === 0 ? (
                ...
                payouts.data.map((payout) => {
                  const isCompleted = payout.status?.toLowerCase() === 'completed' || payout.status?.toLowerCase() === 'success';
                  return (
                    <tr key={payout.id} className="text-default-700 text-sm hover:bg-default-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-[11px] uppercase font-bold rounded ${
                          isCompleted ? 'bg-success text-white' : 'bg-default-300 text-default-800'
                        }`}>
                          {isCompleted ? 'SUCCESS' : payout.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {payout.processed_by || '-'}
                      ...
                      <td className="px-6 py-4 text-default-500 whitespace-nowrap">
                        {payout.stripe_transfer_id || payout.id || '-'}
                      </td>
