...
const UserListTabel = () => {
  ...
  useEffect(() => {
    const fetchAffiliates = async () => {
      ...
      try {
        ...
        // Include status if they optionally try to filter it from future extensions
        if (status) queryParams.append('status', status);

        const result = await apiFetch(`/api/v1/admin/affiliates/?${queryParams}`, { token });
  ...
  return (
    <div className="card">
              ...
              <table className="min-w-full divide-y divide-default-200">
                <thead className="bg-default-150">
                  <tr className="text-sm font-normal text-default-700 whitespace-nowrap">
                    <th className="px-3.5 py-3 text-start">Affiliate ID</th>
                    ...
                    <th className="px-3.5 py-3 text-start">Total Commission</th>
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan="9" className="px-3.5 py-8 text-center text-default-500">
                        ...
                        <span className="mt-2 block">Loading affiliates...</span>
                      </td>
                    </tr>
                  ) : data.length === 0 ? (
                    <tr>
                    ...
                    data.map((affiliate) => (
                      <tr key={affiliate.id} className="text-default-800 font-normal text-sm whitespace-nowrap hover:bg-default-50 transition-colors">
                        <td className="px-3.5 py-3 text-primary">#{affiliate.id}</td>
                        <td className="flex py-3 px-3.5 items-center gap-3">
                          <div className="w-10 h-10 flex items-center justify-center rounded-full bg-primary/10 text-primary font-semibold text-lg uppercase">
                            {affiliate.name ? affiliate.name.charAt(0) : 'A'}
                          </div>
                          <div>
                            <h6 className="mb-0 font-semibold text-default-800">
                              {affiliate.name || 'Unknown Affiliate'}
                            </h6>
                          </div>
                        </td>
                        <td className="py-3 px-3.5">{affiliate.phone_number || 'N/A'}</td>
                        <td className="py-3 px-3.5">{affiliate.stripe_account_id || 'N/A'}</td>
                        <td className="py-3 px-3.5 font-medium text-warning">
                          ${typeof affiliate.pending_commission === 'number' ? affiliate.pending_commission.toFixed(2) : '0.00'}
                        </td>
                        <td className="py-3 px-3.5 font-medium text-success">
                          ${typeof affiliate.total_commission === 'number' ? affiliate.total_commission.toFixed(2) : '0.00'}
                        </td>
                        <td className="py-3 px-3.5">{formatDate(affiliate.date_joined)}</td>
                        ...
                        <td className="px-3.5 py-3 text-center">
                          <div className="hs-dropdown relative inline-flex">
                            ...
                            <div className="hs-dropdown-menu" role="menu">
                              <Link href={`/${affiliate.role === 'Driver' ? 'drivers-list' : 'riders-list'}/${affiliate.id}`} className="flex items-center gap-1.5 py-1.5 px-3 text-default-500 hover:bg-default-150 rounded">
                                <LuUser className="size-3" /> Overview
                              </Link>
                              <Link href={`/affiliates-list/${affiliate.id}`} className="flex items-center gap-1.5 py-1.5 px-3 text-default-500 hover:bg-default-150 rounded">
                                <LuCar className="size-3" /> History
                              </Link>
                              <button onClick={() => handleEditClick(affiliate)} disabled={fetchingEditId === affiliate.id} className="w-full flex items-center gap-1.5 py-1.5 px-3 text-default-500 hover:bg-default-150 rounded disabled:opacity-50">
                                {fetchingEditId === affiliate.id ? <LuLoader className="size-3 animate-spin" /> : <LuSquarePen className="size-3" />} Edit
                              </button>
                            </div>
                          </div>
                        </td>
      ...
      {/* <AddAffiliateModal isOpen={isAddModalOpen} ... /> */}
