...
import { LuChevronLeft, LuChevronRight, LuSearch, LuLoader, LuCircleX, LuArrowDown, LuArrowUp, LuCopy } from 'react-icons/lu';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const result = await apiFetch(`/api/v1/admin/system/auth-tokens/?${queryParams}`, { token });
    ...
    <div className="card">
      <div className="p-4 flex flex-wrap items-center gap-4">
        ...
        <div className="relative w-72">
          <input type="text" className="form-input form-input-sm ps-9 w-full" placeholder="Search user or token key..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
            ...
            <table className="min-w-full divide-y divide-default-200">
              <thead className="bg-default-150 sticky top-0 z-20 shadow-sm backdrop-blur-md">
                <tr className="text-sm font-normal text-default-700 whitespace-nowrap">
                  <th className="ps-4 py-3 text-start">User</th>
                  <th className="px-3.5 py-3 text-start cursor-pointer select-none" onClick={() => handleSort('key')}>Token Key {currentSortIcon('key')}</th>
                  <th className="px-3.5 py-3 text-start cursor-pointer select-none" onClick={() => handleSort('created')}>Created At {currentSortIcon('created')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-default-200">
                {loading ? (
                  <tr><td colSpan="3" className="py-10 text-center"><LuLoader className="animate-spin size-6 text-primary mx-auto" /><p className="mt-2 text-default-500">Loading Auth Tokens...</p></td></tr>
                ) : data.length === 0 ? (
                  <tr><td colSpan="3" className="py-10 text-center text-default-500">No Auth Tokens found.</td></tr>
                ) : (
                  data.map((row) => (
                    <tr key={row.key || row.id} className="text-default-800 font-normal text-sm hover:bg-default-50 transition-colors">
                      <td className="ps-4 py-3">
                        <div className="flex flex-col">
                          <span className="font-semibold">{(row.user_info || row.user)?.name || row.user_name || 'Unknown'}</span>
                          <span className="text-xs text-default-500">{(row.user_info || row.user)?.phone_number || (row.user_info || row.user)?.email || row.user_email || 'N/A'}</span>
                        </div>
                      </td>
                      <td className="px-3.5 py-3 text-default-600 font-mono tracking-wider max-w-xs relative group">
                        <div className="flex items-center gap-2">
                          <span className="truncate" title={row.key}>{row.key}</span>
                          <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(row.key); }} className="text-default-400 hover:text-primary transition-colors flex-shrink-0" title="Copy Token Key">
                            <LuCopy className="size-4" />
                          </button>
                        </div>
                      </td>
                      <td className="px-3.5 py-3 whitespace-nowrap text-default-600 text-xs">{formatDate(row.created || row.created_at)}</td>
                    </tr>
                  ))
                )}
              </tbody>
...
export default AuthTokensTable;
